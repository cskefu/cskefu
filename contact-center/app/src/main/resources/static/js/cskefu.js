...
function newMessageScorllBottom(type, msgType) {
    var box = type == "agent" ? 'chat_msg_list' : 'chat_msg_list_cca';
    if (msgType == "image") {
        chatScorllBottom(box)
    } else {
        document.getElementById(box).scrollTop = document.getElementById(box).scrollHeight
    }
}
    ...
    newAgentUserMessage: function (data, type) {
        if (data.usession == cursession) {
            ...
            if (data.message != "") {
                ...
                data.cususerid = cususerid;
                data.headimgurl = curHeadImgUrl;
