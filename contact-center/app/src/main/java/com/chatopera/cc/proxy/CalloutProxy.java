...
public class CalloutProxy {
    ...
    public static UKefuCallOutConfig initCallOutConfig(String dataid, String orgi) {
        UKefuCallOutConfig ukefuCallOutConfig = MainContext.getCache().findOneSystemByIdAndOrgi(Constants.SYSTEM_CACHE_CALLOUT_CONFIG + "_" + dataid, orgi);
        if (MainContext.getContext() != null && ukefuCallOutConfig == null) {
            ...
            if (ukefuCallOutConfigList.size() == 0) {
                ...
                MainContext.getCache().putSystemByIdAndOrgi(Constants.SYSTEM_CACHE_CALLOUT_CONFIG + "_" + ukefuCallOutConfig.getDataid(), orgi, ukefuCallOutConfig);
            }
        }
    ...
    public static UKefuCallOutConfig initCallOutConfig(String orgi) {
        UKefuCallOutConfig ukefuCallOutConfig = MainContext.getCache().findOneSystemByIdAndOrgi(Constants.SYSTEM_CACHE_CALLOUT_CONFIG + "_" + orgi, orgi);
        if (MainContext.getContext() != null && ukefuCallOutConfig == null) {
            UKefuCallOutConfigRepository ukefuCallOutConfigRepository = MainContext.getContext().getBean(UKefuCallOutConfigRepository.class);
            ...
            if (ukefuCallOutConfigList.size() == 0) {
                ...
                ukefuCallOutConfig = ukefuCallOutConfigList.get(0);
                MainContext.getCache().putSystemByIdAndOrgi(Constants.SYSTEM_CACHE_CALLOUT_CONFIG + "_" + orgi, orgi, ukefuCallOutConfig);
            }
        }
        return ukefuCallOutConfig;
    }
    ...
    public static UKefuCallOutNames processNames(UKDataBean name, CallCenterAgent agent, String orgi, int leavenames) {
        ...
        List<UKefuCallOutNames> callNamesList = callOutNamesRes.findByDataidAndCreaterAndOrgi((String) name.getValues().get("id"), (String) name.getValues().get(Constants.CSKEFU_SYSTEM_DIS_AGENT), orgi);
        ...
        if (callOutName != null) {
            ...
            callOutName.setStatus(MainContext.NamesProcessStatusEnum.DIS.toString());

            callOutName.setCreater((String) name.getValues().get(Constants.CSKEFU_SYSTEM_DIS_AGENT));
            callOutName.setOrgan((String) name.getValues().get(Constants.CSKEFU_SYSTEM_DIS_ORGAN));
            ...
            callOutName.setOwneruser((String) name.getValues().get(Constants.CSKEFU_SYSTEM_DIS_AGENT));
