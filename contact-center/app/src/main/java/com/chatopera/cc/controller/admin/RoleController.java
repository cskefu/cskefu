...
public class RoleController extends Handler {
    ...
    public ModelAndView index(ModelMap map, HttpServletRequest request, @Valid String role, @Valid String msg) {
        ...
        if (roleList.size() > 0) {
            ...
            if (roleData != null) {
                map.addAttribute("userRoleList", userRoleRes.findByOrgiAndRole(super.getOrgiByTenantshare(request), roleData, PageRequest.of(super.getP(request), super.getPs(request))));
            }
        }
