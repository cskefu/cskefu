...
public class CalloutWireTask {
    ...
    private String bindAgentService(final AgentUser agentUser,
        ...
        statusEvent.setCalltype(MainContext.CallCenterCallType.OUTSIDELINE.toString());
        statusEvent.setDirection(MainContext.CallType.OUT.toString());
        ...
        statusEvent.setAgent(agentUser.getAgentno());
        ...
        as.setQualitystatus(MainContext.QualityStatusEnum.NO.toString()); // 不做质检
...
//            HttpServletRequest request,
            ...
            final String sip,
            final Date createtime,
            final String callid) throws CSKefuException, CharacterCodingException {
        // Define source
        ...
        if (onlineUser == null) {
            ...
            onlineUser.setUsertype(MainContext.OnlineUserType.TELECOM.toString());

//            if(request.getSession().getAttribute("Sessionusername") != null){
//                String str=(String)request.getSession().getAttribute("Sessionusername");
//                onlineUser.setUsername(str);
//            }else{
//                onlineUser.setUsername(MainContext.GUEST_USER + "_" + MainUtils.genIDByKey(onlineUser.getId()));
//            }
            AgentUser agentUser = this.agentUserRepository.findOneByUserid(onlineUser.getUserid());
            if (agentUser != null) {
                onlineUser.setUsername(agentUser.getUsername());
            } else {
                onlineUser.setUsername(Constants.GUEST_USER + "_" + MainUtils.genIDByKey(onlineUser.getId()));
            }
            ...
            onlineUser.setChannel(MainContext.ChannelType.PHONE.toString()); // telcom
            ...
            onlineUser.setOptype(MainContext.OptType.HUMAN.toString());
...
//        }
        ...
        AgentUser agentUser = new AgentUser(onlineUser.getId(),
                                            MainContext.ChannelType.PHONE.toString(), // callout
                                            onlineUser.getId(),
        ...
        final User agent = userRes.findOne(agentId);
    ...
    public void offline(final CalloutWireEvent event) throws Exception {
        ...
        if (onlineUser != null) {
            ...
            for (AgentUser au : agentUsers) {
                cache.deleteAgentUserByUserIdAndOrgi(au.getId(), MainContext.SYSTEM_ORGI);
    ...
    public void callOutConnect(final CalloutWireEvent event) throws FreeSwitchException, CSKefuException, CalloutRuntimeException, CharacterCodingException {
        ...
        CousultInvite invite = OnlineUserProxy.consult(channel, MainContext.SYSTEM_ORGI);
        if (invite == null) {
        ...
        if (StringUtils.isNotBlank(sip)) { // 发送通知给浏览器客户端
            ...
            payload.addProperty("calltype", MainContext.CallType.OUT.toString());
            ...
            NettyClients.getInstance()
                    .sendCalloutEventMessage(agent.getId(),
                                             MainContext.MessageType.NEW.toString(),
                                             payload.toString());
        } else {
