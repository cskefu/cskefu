 ...
 */
package com.chatopera.cc.app.service.impl;
...
public class ESDataExchangeImpl{
	...
	public IndexRequestBuilder saveBulk(UKDataBean dataBean) throws Exception {
		...
		if(!StringUtils.isBlank(dataBean.getType())) {
			indexRequestBuilder = MainContext.getTemplet().getClient().prepareIndex(MainContext.SYSTEM_INDEX,
	...
	public void deleteByCon(QueryBuilder query ,String type) throws Exception {
	    ...
	    SearchResponse response = MainContext.getTemplet().getClient().prepareSearch(MainContext.SYSTEM_INDEX).setTypes(type)
	...
	public void deleteById(String type , String id){
		if(!StringUtils.isBlank(type) && !StringUtils.isBlank(id)){
			...
			.prepareDelete(MainContext.SYSTEM_INDEX, type, id).execute().actionGet();
		}
	}
	...
	public UKDataBean getIObjectByPK(UKDataBean dataBean , String id) {
		if(dataBean.getTable()!=null){
			GetResponse getResponse = MainContext.getTemplet().getClient()
					.prepareGet(MainContext.SYSTEM_INDEX,
							dataBean.getTable().getTablename(), dataBean.getId())
					.execute().actionGet();
	...
	public void updateIObject(UKDataBean dataBean) throws Exception {
		...
		MainContext.getTemplet().getClient()
				.prepareUpdate(MainContext.SYSTEM_INDEX,
	...
	public PageImpl<UKDataBean> findAllPageResult(QueryBuilder query,String index ,MetadataTable metadata, Pageable page , boolean loadRef , String types) {
		...
		SearchRequestBuilder searchBuilder = MainContext.getTemplet().getClient().prepareSearch(MainContext.SYSTEM_INDEX);
		...
		for(SearchHit hit : response.getHits().getHits()){
			...
			if(loadRef == true) {
				if(!StringUtils.isBlank((String)temp.getValues().get(MainContext.UKEFU_SYSTEM_DIS_AGENT))) {
					users.add((String)temp.getValues().get(MainContext.UKEFU_SYSTEM_DIS_AGENT)) ;
				}
				if(!StringUtils.isBlank((String)temp.getValues().get(MainContext.UKEFU_SYSTEM_ASSUSER))) {
					users.add((String)temp.getValues().get(MainContext.UKEFU_SYSTEM_ASSUSER)) ;
				}
				if(!StringUtils.isBlank((String)temp.getValues().get(MainContext.UKEFU_SYSTEM_DIS_ORGAN))) {
					organs.add((String)temp.getValues().get(MainContext.UKEFU_SYSTEM_DIS_ORGAN)) ;
				}
		...
		if(loadRef) {
			if(users.size() > 0) {
				...
				for(UKDataBean dataBean : dataBeanList) {
					String userid = (String)dataBean.getValues().get(MainContext.UKEFU_SYSTEM_DIS_AGENT) ;
					...
					String assuer = (String)dataBean.getValues().get(MainContext.UKEFU_SYSTEM_ASSUSER) ;
			...
			if(organs.size() > 0) {
				...
				for(UKDataBean dataBean : dataBeanList) {
					String organid = (String)dataBean.getValues().get(MainContext.UKEFU_SYSTEM_DIS_ORGAN) ;
	...
	public PageImpl<UKDataBean> findAllPageAggResult(QueryBuilder query,String aggField,Pageable page , boolean loadRef , String types) {
		...
		if(response.getAggregations().get(aggField) instanceof Terms){
			...
			if(agg!=null){
				if(loadRef == true) {
					if(aggField.equals(MainContext.UKEFU_SYSTEM_DIS_AGENT)) {
						users.add(agg.getName()) ;
					}
					if(aggField.equals(MainContext.UKEFU_SYSTEM_DIS_ORGAN)) {
