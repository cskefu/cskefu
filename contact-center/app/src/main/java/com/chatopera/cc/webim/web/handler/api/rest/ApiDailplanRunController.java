...
import com.chatopera.cc.webim.service.repository.CallOutDialplanRepository;
import com.chatopera.cc.webim.service.repository.UserRepository;
import com.chatopera.cc.webim.service.task.CallOutPlanTask;
import com.chatopera.cc.webim.web.handler.Handler;
import com.chatopera.cc.webim.web.handler.api.request.RestUtils;
import com.chatopera.cc.webim.web.model.CallOutDialplan;
...
public class ApiDailplanRunController extends Handler {
    ...
    protected JsonObject execute(final CallOutDialplan dp) {
        ...
        if (dp.isIsarchive()) {
        ...
        } else if (dp.getStatus().equals(UKDataContext.CallOutDialplanStatusEnum.STOPPED.toString())) {

            // 查询该技能组的SIP号码列表
            final JsonArray sips = getSipsByOrgan(dp.getOrgan().getId(), UKDataContext.SYSTEM_ORGI);
            ...
            if (concurrency >= 1) {
                ...
                // 非暂停中
                dp.setStatus(UKDataContext.CallOutDialplanStatusEnum.RUNNING.toString());
                ...
                if (existed != null) {
                    ...
                    // 非RUNNING的状态
                    if (pre.has("status") && !(pre.get("status").getAsString().equals(UKDataContext.CallOutDialplanStatusEnum.RUNNING.toString()))) {
                        logger.info("[callout api] 从暂停状态恢复到启动 {} {}", dp.getName(), dp.getId());

                        // 设置持久化状态
                        pre.addProperty("concurrency", concurrency);
                        pre.addProperty("status", UKDataContext.CallOutDialplanStatusEnum.RUNNING.toString());
    ...
    private JsonObject pause(final CallOutDialplan dp) {
        JsonObject resp = new JsonObject();
        if (dp.getStatus().equals(UKDataContext.CallOutDialplanStatusEnum.RUNNING.toString())) {
            JsonObject payload = new JsonObject();
            payload.addProperty("dialplan", dp.getId());
            ...
            payload2.addProperty("status", UKDataContext.CallOutDialplanStatusEnum.STOPPED.toString());
            ...
            dp.setUpdatetime(dt);
            dp.setStatus(UKDataContext.CallOutDialplanStatusEnum.STOPPED.toString());
            callOutDialplanRes.save(dp);
            resp.addProperty(RestUtils.RESP_KEY_RC, RestUtils.RESP_RC_SUCC);
