...
import com.chatopera.cc.util.event.CallOutWireEvent;
import com.chatopera.cc.util.exception.CSKefuException;
import com.chatopera.cc.util.exception.CallOutRuntimeException;
import com.chatopera.cc.util.exception.FreeSwitchException;
...
public class CallOutWireTask implements MessageListener {
    ...
    protected AgentStatus getAgentStatus(final String agentId, final String orgi) {
        List<AgentStatus> x = agentStatusRes.findByAgentnoAndOrgi(agentId, UKDataContext.SYSTEM_ORGI);
        if (x.size() > 0)
            return x.get(0);
        return null;
    }
    ...
    private String bindAgentService(final AgentUser agentUser,
        ...
        statusEvent.setCalltype(UKDataContext.CallCenterCallTypeEnum.OUTSIDELINE.toString());
        statusEvent.setDirection(UKDataContext.CallTypeEnum.OUT.toString());
        ...
        statusEvent.setStatus(UKDataContext.CallServiceStatus.INCALL.toString());
        ...
        as.setQualitystatus(UKDataContext.QualityStatus.NO.toString()); // 不做质检
        as.setInitiator(UKDataContext.ChatInitiatorType.USER.toString());
        as.setName(agentUser.getUsername());
        as.setDataid(agentUser.getUserid());
        as.setSessiontype(UKDataContext.AgentUserStatusEnum.INSERVICE.toString());
    ...
    private AgentUser online(final String visitorPhoneNumber,
        ...
        // 创建在线访客
        OnlineUser onlineUser = onlineUserRes.findByPhoneAndOrgi(visitorPhoneNumber, UKDataContext.SYSTEM_ORGI);
        if (onlineUser == null) {
            onlineUser = new OnlineUser();
            onlineUser.setId(UKTools.getUUID());
            onlineUser.setUserid(onlineUser.getId());
            onlineUser.setUsertype(UKDataContext.OnlineUserTypeStatus.TELECOM.toString());
            onlineUser.setUsername(UKDataContext.GUEST_USER + "_" + UKTools.genIDByKey(onlineUser.getId()));
            onlineUser.setOrgi(UKDataContext.SYSTEM_ORGI);
            ...
            onlineUser.setChannel(UKDataContext.ChannelTypeEnum.PHONE.toString()); // telcom
        ...
        CacheHelper.getOnlineUserCacheBean().put(onlineUser.getId(), onlineUser, UKDataContext.SYSTEM_ORGI);


        // 在线客服状态
//        AgentStatus agentStatus = getAgentStatus(agentId, UKDataContext.SYSTEM_ORGI);
//
//        if (agentStatus == null) {
...
//        }
        ...
        AgentUser agentUser = new AgentUser(onlineUser.getId(),
                UKDataContext.ChannelTypeEnum.PHONE.toString(), // callout
                onlineUser.getId(),
        ...
        CacheHelper.getAgentUserCacheBean().put(agentUser.getId(), agentUser, UKDataContext.SYSTEM_ORGI);
        return agentUser;
    }
    ...
    public void offline(final CallOutWireEvent event) throws Exception {
        OnlineUser onlineUser = onlineUserRes.findByPhoneAndOrgiAndStatus(event.getTo(),
                UKDataContext.SYSTEM_ORGI,
                UKDataContext.OnlineUserOperatorStatus.ONLINE.toString());
        Date dt = new Date();

        if (onlineUser != null) { // 已经接通的用户
            // update Online user
            CacheHelper.getOnlineUserCacheBean().delete(onlineUser.getId(), UKDataContext.SYSTEM_ORGI);
            ...
            List<AgentUser> agentUsers = agentUserRes.findByUseridAndStatus(onlineUser.getId(),
                    UKDataContext.AgentUserStatusEnum.INSERVICE.toString());
            for (AgentUser au : agentUsers) {
                CacheHelper.getAgentUserCacheBean().delete(au.getId(), UKDataContext.SYSTEM_ORGI);
                ServiceQuene.serviceFinish(au, UKDataContext.SYSTEM_ORGI);
    ...
    private void closeStatusEvent(AgentUser au, final Date endtime, final String recordingfile) {
        ...
        if (se != null) {
            se.setStatus(UKDataContext.CallServiceStatus.HANGUP.toString());
    ...
    public void callOutConnect(final CallOutWireEvent event) throws FreeSwitchException, CSKefuException, CallOutRuntimeException {
        ...
        CousultInvite invite = OnlineUserUtils.cousult(channel, UKDataContext.SYSTEM_ORGI, consultInviteRes);
        if (invite == null) {
        ...
        if (StringUtils.isNotBlank(sip)) { // 发送通知给浏览器客户端
            logger.info("[callout wire] push event to sip account {} ...", sip);
            ...
            payload.addProperty("calltype", UKDataContext.CallTypeEnum.OUT.toString());
            ...
            NettyClients.getInstance()
                    .sendCalloutEventMessage(agent.getId(),
                            UKDataContext.MessageTypeEnum.NEW.toString(),
                            payload.toString());
        } else {
