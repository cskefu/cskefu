...
import com.chatopera.cc.schedule.CalloutPlanTask;
...
public class ApiDailplanRunController extends Handler {
    ...
    private JsonArray getSipsByOrgan(final String organ, final String orgi) {
        ...
        List<String> sips = UserProxy.findSipsByOrganAndDatastatusAndOrgi(organ, false, orgi);
        for (String sip : sips) {
    ...
    protected JsonObject execute(final CallOutDialplan dp) {
        ...
        if (dp.isIsarchive()) {
            ...
            final long countagent = UserProxy.countByOrgiAndAgentAndDatastatusAndOrgan(MainContext.SYSTEM_ORGI, true, false, dp.getOrgan().getId());
            ...
            if (concurrency >= 1) {
                ...
                if (existed != null) {
                    ...
                    // 非RUNNING的状态
                    if (pre.has("status") && !(pre.get("status").getAsString().equals(
                            MainContext.CalloutDialplanStatusEnum.RUNNING.toString()))) {
