...
import com.chatopera.cc.proxy.CalloutProxy;
    ...
    public void run() {
        if (agent != null) {
            ...
            if (names != null && names.getContent().size() > 0) {
                ...
                CalloutProxy.processNames(name, agent, agent.getOrgi(), (int) (names.getTotalElements() - 1));
            } else {
            ...
            NamesTask.getCache().putCallCenterAgentByIdAndOrgi(agent.getUserid(), agent.getOrgi(), agent);
        }
    }
