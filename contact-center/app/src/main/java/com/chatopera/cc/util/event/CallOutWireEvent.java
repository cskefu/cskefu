 ...
 */
package com.chatopera.cc.util.event;

import com.chatopera.cc.core.UKDataContext;
import com.chatopera.cc.core.UKDataContext.CallTypeEnum;
...
//    }
    ...
    public static CallOutWireEvent parse(final JsonObject j) throws CallOutRuntimeException {
        ...
        if (j.has("ops")) {
            ...
            switch (ops) {
                case "hangup":
                    c.setStatus(UKDataContext.CallServiceStatus.HANGUP.toString());
                    break;
                case "answer":
                    c.setStatus(UKDataContext.CallServiceStatus.INCALL.toString());
                    break;
                default:
                    // 其他类型的问题
                    break;
        ...
        if (CallTypeEnum.IN.toString().equals(c.getDirection())) { // 呼入
            // 暂不处理
            if (UKDataContext.CallServiceStatus.INCALL.toString().equals(c.getStatus())) {
                // 呼入应答
                c.setEventType(UKDataContext.CallWireEventType.CALLIN_CONN.getIndex());
            } else if (UKDataContext.CallServiceStatus.HANGUP.toString().equals(c.getStatus())) {
                if (c.getTo() != null) {
                    // 呼入挂断
                    c.setEventType(UKDataContext.CallWireEventType.CALLIN_DIST.getIndex());
                } else {
                    // 呼入失败
                    c.setEventType(UKDataContext.CallWireEventType.CALLIN_FAIL.getIndex());
                }
            }
            ...
            if (c.isDialplan()) { // 自动外呼
                if (UKDataContext.CallServiceStatus.INCALL.toString().equals(c.getStatus())) {
                    // 自动外呼应答
                    c.setEventType(UKDataContext.CallWireEventType.DIALPLAN_CONN.getIndex());
                } else if (UKDataContext.CallServiceStatus.HANGUP.toString().equals(c.getStatus())) {
                    if (c.getFrom() == null) {
                        // 自动外呼失败
                        c.setEventType(UKDataContext.CallWireEventType.DIALPLAN_FAIL.getIndex());
                    } else {
                        // 自动外呼挂断
                        c.setEventType(UKDataContext.CallWireEventType.DIALPLAN_DISC.getIndex());
                    }
                }
            } else { // 手动外呼
                if (UKDataContext.CallServiceStatus.INCALL.toString().equals(c.getStatus())) {
                    // 手动外呼应答
                    c.setEventType(UKDataContext.CallWireEventType.MANUDIAL_CONN.getIndex());
                } else if (UKDataContext.CallServiceStatus.HANGUP.toString().equals(c.getStatus())) {
                    if (c.getFrom() == null) {
                        // 手动外呼失败
                        c.setEventType(UKDataContext.CallWireEventType.MANUDIAL_FAIL.getIndex());
                    } else {
                        c.setEventType(UKDataContext.CallWireEventType.MANUDIAL_DISC.getIndex());
                    }
                }
            }
        }
